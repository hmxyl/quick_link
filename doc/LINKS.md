...
- **账号管理**：添加/编辑链接弹窗不再包含关联账号面板；所有账号管理操作统一通过列表页操作列的「账号管理」按钮进入"账号详情"弹窗处理——表格形式展示（用户名/邮箱/密码/备注/操作），密码直接可见并可复制，支持逐个删除与追加新账号；增删后自动刷新；无关联账号时弹窗展示空列表与「添加关联账号」入口，可直接新增。
...
- **一链接多账号（内嵌数组）**：同一网址常有多个身份（工作/私人/小号），`accounts` 数组内嵌于链接文档，每条独立 `_id` 支持定向删除；追加/删除走子接口而非整体替换，与基础编辑互不干扰。
